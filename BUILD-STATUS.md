...
**Last updated:** 2026-07-10 (second session)
**Status:** Live in production at [runofpractice.com](https://www.runofpractice.com), single real user (solo coach), actively iterating ahead of August testing. **This session's landing-page/feedback/legal-pages frontend changes are in the working tree but not yet git-committed** (the `feedback` table migration is already applied to the live Supabase project regardless) — check `git status`, not just `git log`, before assuming the frontend has landed. (Earlier same-day session's testing-round-1 work landed in commit `26aaaf1` and is already on `main`.)
...
**Public landing page, feedback, and legal pages** (2026-07-10, second session)
...
- Landing page CTAs ("Try it free" / "Already have an account? Sign in") both lead to the same `AuthScreen` (now with a back button to return to the landing page), just visually weighted differently, plus a "Founding Coach" early-access framing, a 4-capability grid, and an anonymous feedback form.
- `/terms` and `/privacy` — static pages (`src/components/LegalPages.jsx`), hand-transcribed (no markdown-parser dependency added) from the two drafts Jax supplied, draft-notice and manual-deletion disclosure preserved verbatim. Routed via explicit `vercel.json` rewrites (`/live` and `/preview` were the only prior entries — anything not covered there 404s on Vercel's static host, so new client-only routes need an explicit rewrite each time).
- New `feedback` table + `submit_public_feedback` RPC (anon, narrow) — see migration `20260710050000_public_feedback.sql`. In-app path folds into the existing Home "?" button, now a two-item menu ("Getting Started" / "Send Feedback") rather than adding a second icon.
- Sign-in screen now links Terms/Privacy under the Send Code button.
