-- PreviewView (the pre-practice setup screen) needs a "Start Practice"
-- button for a coach who's done getting stations together, but the RPC
-- never returned the practice's id -- only used it internally to build
-- the rest of the response. Re-declaring with one added field; nothing
