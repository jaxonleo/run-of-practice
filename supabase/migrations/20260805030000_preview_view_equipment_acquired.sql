-- Direct feedback: equipment a coach doesn't own yet (assets.acquired=false,
-- see 20260805020000) needs to keep showing up -- and stand out -- on the
-- practice preview screen (/preview/:token), not just inside the app. This
-- RPC ran under security definer and previously returned equipment as bare
-- name strings; switching each item to {name, acquired} lets the client
-- flag "not acquired" gear without a second round trip or exposing anything
-- beyond what was already public on this anonymous-readable view.
