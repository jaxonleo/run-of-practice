-- Direct feedback: the "Up Next" queue in a live practice only showed a
-- drill's name and duration -- a viewer (anonymous helper or coach) had
-- no way to peek at what equipment/location/rotation was coming up
-- without actually advancing the practice. Extends upcoming_activities
-- with the same equipment/location/station detail get_preview_view
-- already returns for the whole plan, plus each station's planned player
-- assignments (ids only, resolved against the roster already returned by
-- this same RPC) so a "player rotations" preview is possible without a
-- second round trip.
