-- Supporting fields for a new "who has this practice open" presence
-- indicator (Realtime Presence, no new table -- pure ephemeral
-- client-to-client state, keyed by practice_id so a coach still in
-- Practice Setup and a helper who already opened the live link show up
-- together). Two things were missing to make that possible client-side:
-- (1) get_preview_view had no way for a signed-in coach viewer to know
-- their OWN display name (it already returns the whole team_staff roster
-- for can_manage viewers, but not which row is "me"); (2)
-- get_live_session_view never returned practice_id at all, which
-- HelperView needs to key the same presence channel the staff view and
-- PreviewView use.
