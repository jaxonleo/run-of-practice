-- Real bug found via live testing (disposable test account, browser
-- click-through): Goals & Insights' "Target vs. Planned vs. Actual" glance
-- view was broken for every team -- get_team_goal_report errored with
-- "column tg.skill_tag_id does not exist".
--
-- team_goals was migrated from skill_tag_id to skill_category_id back on
-- 2026-07-19 (20260719030000, the "goals moved up from individual skill
-- tags to their global category" decision -- see BUILD-STATUS), and
-- 20260719040000_goals_rpcs_category_level.sql already re-defined both
-- set_team_goals and get_team_goal_report against the new column via
-- `create or replace function`. That should have been the end of it, but
-- calling the RPC just now against production hit the *old*
-- skill_tag_id-based body from the migration one before it
-- (20260719010000_goal_report_global_tag_rollup.sql) -- i.e. the later,
-- correct `create or replace` never actually took effect live, despite
-- both migrations showing as applied in the CLI's own history. Given the
-- project's own documented history of migration-history/live-schema drift
-- (see Decision History, thirteenth session), rather than spend more time
-- diagnosing exactly how that happened, this just re-applies both
-- functions' correct, category-based bodies verbatim from
-- 20260719040000_goals_rpcs_category_level.sql -- idempotent, and fixes
-- the live function regardless of the historical cause.
