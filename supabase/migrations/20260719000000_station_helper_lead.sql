-- Live tap-to-reassign needs a station lead that isn't necessarily a
-- roster team_staff row -- a parent helping out isn't added as staff just
-- to stand at a station. Freeform, nullable, and mutually exclusive with
-- team_staff_id at the application layer (picking a roster coach clears
-- this, typing a helper name clears team_staff_id) rather than a DB
-- constraint, same convention as other soft-exclusive fields in this schema.
alter table public.stations add column helper_name text;

-- Both anon-facing views join stations -> team_staff for coach_name and had
-- no fallback, so a station led by a freeform helper showed no name at all
-- on the shared setup link or live helper view. Re-declaring both functions
-- in full (matching the latest bodies from 20260707220000 and
-- 20260707260000) with one added coalesce -- Postgres functions can only be
-- replaced whole, not patched.
