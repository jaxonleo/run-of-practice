-- Grouping stations by position/handedness ("Group By...") previously threw
-- away *why* a station's players were grouped that way once the drill went
-- live -- a coach running a station had no way to tell "this group is the
-- lefties" without re-checking the builder. Store the label the grouping
-- produced (e.g. "Lefties", "C") right on the station it was assigned to,
-- same convention as the assignments column added alongside it.
alter table public.stations add column group_label text;

-- Re-declare get_live_session_view (latest body from 20260719000000) with
-- group_label added to the stations jsonb -- Postgres functions can only be
-- replaced whole, not patched. HelperView matches groups to stations by
-- position/group_number client-side, same as the coach view, so the label
-- just needs to ride along on each station row.
