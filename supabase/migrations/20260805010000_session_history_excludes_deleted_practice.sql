-- Direct feedback: Goals & Insights' Practice History had no way to delete
-- a completed practice, even though "Delete" already exists elsewhere in
-- the app (Home's practice ellipsis menu) via a plain archivePractice()
-- (practices.archived_at, the app-wide soft-delete convention -- see
-- BUILD-STATUS's "almost no DELETE policy anywhere" note). That existing
-- archive call alone wouldn't actually remove a session from this list,
-- though: get_team_session_history's own `sessions` CTE joins
-- practice_live_sessions to practices on team_id/status only, with no
-- archived_at check at all, unlike every other practice-listing query in
-- the app. Adding the same filter here is what makes "Delete" from History
-- actually take effect, rather than silently leaving the deleted practice's
...
begin
  ...
  with sessions as (
    ...
    where p.team_id = p_team_id and pls.status = 'completed' and p.archived_at is null
