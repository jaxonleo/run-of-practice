-- Direct feedback: the coach's own live Overview panel shows every activity
-- in the practice, with anything already run crossed out (i<idx, purely a
-- local-plan computation on the coach's side, since the coach already has
-- the full plan client-side). HelperView has no local plan to compute that
-- from -- get_live_session_view only ever returned upcoming_activities
-- (position > current), so a helper's Overview could never show completed
-- activities at all, not just a rendering gap. Adds a new all_activities
-- field alongside the existing upcoming_activities (left untouched, still
-- used for the "Up Next" queue) -- same shape, every activity regardless
-- of position, each tagged with its own `position` so the client can
-- compare against current_practice_activity_id's position the same way
-- the coach's own idx/i<idx check already does.
