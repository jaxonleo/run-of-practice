-- Practice History "unreviewed note" indicator: a coach should be able to
-- tell at a glance whether a completed session has a note nobody's looked
-- at yet, without opening every session in the list. "Reviewed" is tracked
-- on the note itself (viewed_at), not per-coach -- this app has no concept
-- of per-user read state anywhere else, and a shared viewed_at is the
-- simplest thing that matches how the app is actually used today (see
-- BUILD-STATUS "single real user"). Marking viewed is done as a plain
-- client-side update against this column, same pattern archiveNote()
-- already uses for archived_at -- both are covered by the existing
-- notes_update_archive policy (can_access_practice), so no new RPC or
-- policy is needed for it.
alter table public.notes add column viewed_at timestamptz;

-- get_team_session_history gains has_unviewed_notes: true when the
-- session's practice has at least one non-archived, unviewed note. Cheap
-- correlated EXISTS per session -- this RPC is already capped at p_limit
-- sessions, not worth folding into the CTEs above it.
