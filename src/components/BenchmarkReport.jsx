...
function TeamBenchmarksOverview({ teamId, onOpen }) {
  ...
  if (!report) return <div style={{ fontSize: 13, color: "var(--text-dim)" }}>Loading...</div>;
  if (report.error) return <div style={{ fontSize: 13, color: "var(--text-dim)" }}>Benchmark history is not available for this team.</div>;
  const list = report.benchmarks || [];
  if (!list.length) return <div className="card"><div style={{ fontSize: 13, color: "var(--text-dim)" }}>No benchmarks adopted for this team yet. Add one from Library &rarr; Benchmarks or in the Builder.</div></div>;
        ...
        return (
          <div key={row.team_benchmark_id} className="card" style={{ cursor: "pointer" }} onClick={() => onOpen(row.benchmark.id)}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
              ...
              <div style={{ fontSize: 11, color: "var(--text-dim)" }}>{row.finalized_count} finalized{row.recording_count ? " · " + row.recording_count + " recording" : ""}</div>
            </div>
            <div style={{ fontSize: 12, color: "var(--text-dim)", marginTop: 2 }}>{row.benchmark.subject_mode === "team" ? "Whole team" : "Individual"} · {protocol.metricType}</div>
...
function TeamBenchmarkDetail({ teamId, team, coachId, benchmarkId, canManage, onBack, isDesktop }) {
  ...
  if (!detail) return <div style={{ fontSize: 13, color: "var(--text-dim)" }}>Loading...</div>;
  if (detail.error) return <div><button className="btn ghost bxs" onClick={onBack}>&larr; Back</button><div style={{ fontSize: 13, color: "var(--text-dim)", marginTop: 8 }}>Not available.</div></div>;
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 12, color: "var(--text-dim)", marginBottom: 10 }}>{subjectMode === "team" ? "Whole-team" : "Individual"} · {protocol.metricType} · {(detail.versions || []).length} version{(detail.versions || []).length === 1 ? "" : "s"}</div>
      ...
      {!latest && <div className="card"><div style={{ fontSize: 13, color: "var(--text-dim)" }}>No finalized assessments yet.</div></div>}
      ...
      {latest && <div className="card" style={{ marginBottom: 10 }}>
        ...
        {cur && !cur.collective && cur.teamPerf && !cur.teamPerf.noResults && <>
          ...
          {protocol.metricType === "score_rubric" && cur.teamPerf.byLevel && <div style={{ fontSize: 12, color: "var(--text-dim)", marginTop: 2 }}>
      ...
      {latest && elig.length > 1 && <div className="card" style={{ marginBottom: 10 }}>
        ...
        {compareId === "season" && !compare && <div style={{ fontSize: 12, color: "var(--caution)", marginBottom: 6 }}>Baseline unavailable (archived or excluded). Choose another below or set a new one.</div>}
        {!improvement && <div style={{ fontSize: 13, color: "var(--text-dim)" }}>No comparable assessment (version mismatch or excluded).</div>}
        ...
        {improvement && improvement.status === "ok" && improvement.kind !== undefined && <div>
          ...
          {improvement.note === "few_comparable_players" && <div style={{ fontSize: 12, color: "var(--caution)", marginTop: 4 }}>Only {improvement.matchedCount} comparable player{improvement.matchedCount === 1 ? "" : "s"} — not a team-wide claim.</div>}
          {improvement.improved != null && improvement.kind !== "rubric" && <div style={{ fontSize: 12, color: "var(--text-dim)", marginTop: 4 }}>{improvement.improved} improved · {improvement.unchanged} unchanged · {improvement.worse} lower</div>}
        </div>}
        {improvement && improvement.status === "ok" && improvement.label === "team_challenge" && <div>
          ...
          {improvement.compositionWarning && <div style={{ fontSize: 12, color: "var(--caution)", marginTop: 4 }}>Different participants ({improvement.previousCount ?? "?"} &rarr; {improvement.currentCount ?? "?"}) — not same-player development.</div>}
        </div>}
      </div>}
      ...
      {series.length > 1 && <div className="card" style={{ marginBottom: 10 }}>
        ...
        <table style={{ width: "100%", fontSize: 12, marginTop: 8, borderCollapse: "collapse" }}>
          <thead><tr><th style={{ textAlign: "left", borderBottom: "1px solid var(--border)" }}>Date</th><th style={{ textAlign: "right", borderBottom: "1px solid var(--border)" }}>Average</th><th style={{ textAlign: "right", borderBottom: "1px solid var(--border)" }}>N</th></tr></thead>
      ...
      <div className="card">
        ...
        {all.map(a => <div key={a.id} style={{ fontSize: 13, padding: "5px 0", borderTop: "1px solid var(--border)", display: "flex", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}>
  ...
  return (
    <div className="movly" style={{ zIndex: 330 }} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal" style={{ maxWidth: 620, maxHeight: "88vh", display: "flex", flexDirection: "column" }} onClick={e => e.stopPropagation()}>
        ...
        {!assessmentId && <>
          <div style={{ fontSize: 12, color: "var(--text-dim)", marginBottom: 10 }}>Records a standalone measurement for {team ? team.name : "this team"}. It adds no practice minutes and creates no practice.</div>
...
function Sparkline({ points, lower }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} role="img" aria-label="Assessment averages over time">
      <path d={d} fill="none" stroke="var(--field)" strokeWidth="2" />
      {points.map((p, i) => <circle key={i} cx={x(i)} cy={y(p.value)} r="2.5" fill={p.excluded ? "var(--text-dim)" : "var(--field)"} />)}
    </svg>
  );
}
...
export function PlayerBenchmarks({ teamId, playerId }) {
  ...
  if (!list.length) return <div style={{ fontSize: 13, color: "var(--text-dim)" }}>No benchmark results for this player yet.</div>;
        ...
        return Object.entries(obsByVersion).map(([vid, obs]) => {
          ...
          return (
            <div key={b.benchmark.id + vid} className="card">
              ...
              {!latest && <div style={{ fontSize: 13, color: "var(--text-dim)", marginTop: 4 }}>Pending — no complete finalized result yet.</div>}
              {latest && <>
                <div style={{ fontSize: 15, fontWeight: 800, marginTop: 4 }}>{fmtResult(protocol, latest.result)} <span style={{ fontSize: 12, fontWeight: 400, color: "var(--text-dim)" }}>· {latest.date}</span></div>
                ...
                <div style={{ fontSize: 12, marginTop: 4 }}>
                  ...
                  {pb && pbClass.status === "below" && <span style={{ color: "var(--text-dim)" }}>PB {fmtResult(protocol, pb)}</span>}
                </div>
                <div style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 6 }}>{officials.length} finalized observation{officials.length === 1 ? "" : "s"}</div>
              </>}
            </div>
          );
        });
